...
(defun form-pair (strings-array)
    (if (> (list-length strings-array) 0)
        ...
        (if (parse-integer (car (last strings-array)))
            ...
            (cons (intern (string-trim " " (format NIL "~{~A ~}"(list-without-last-element strings-array)))) (parse-integer (car (last strings-array))))
            NIL
        )
        NIL
    )
...
;; Predicat
(defun sort-dsec-helper (list-a list-b)
...
(defun sort-asec-helper (list-a list-b)
    (if (and (listp list-a) (listp list-b))
        (< (cdr list-a) (cdr list-b))
        NIL
    )
